...
class I18NJsonTranslationsPopulator implements II18NTranslationsPopulator {
	...
	public function populate($oComponent) {
		...
		foreach(CodeBase::getLibraries() as $oLibrary) {
			...
			foreach(LanguageEnum::getInstance()->getKeys() as $lang) {
				$path = sprintf('i18n/%s.json', strtolower($lang));
				if ($oLibrary->exists($path)) {
					try {
						$arr = $oLibrary->getResource($path)->parseJSON();
